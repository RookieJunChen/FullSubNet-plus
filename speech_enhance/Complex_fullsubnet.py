...
from audio_zen.model.module.sequence_model import SequenceModel, Complex_SequenceModel, _print_networks

# for log
from utils.logger import log
import fullsubnet_net
import datetime
print = log
...
if __name__ == "__main__":
    ...
    with torch.no_grad():
        model = Model(
            ...
            sb_model_hidden_size=192,
            weight_init=False,
            norm_type="offline_laplace_norm",
            num_groups_in_drop_band=2,
        )

        model2 = fullsubnet_net.Model(
            sb_num_neighbors=15,
            fb_num_neighbors=0,
            num_freqs=257,
            look_ahead=2,
        ...
        ipt = torch.rand(3, 2, 257, 200).to(device)
        model.to(device)
        model2.to(device)
        _print_networks([model, ])
        _print_networks([model2, ])
        start = datetime.datetime.now()
        model(ipt)
        end = datetime.datetime.now()
        print(f"{end - start}")
        print("")
        ipt, _ = torch.chunk(ipt, chunks=2, dim=1)
        start = datetime.datetime.now()
        model2(ipt)
        end = datetime.datetime.now()
        print(f"{end - start}")
