...
from fullsubnet_plus.model.Amp_Attention_fullsubnet import FullSub_Att_FullSubNet
from fullsubnet_plus.model.fullbandnet import FullBandNet, Complex_FullBandNet
...
class Two_Stage_Residual_FullSubNet_Large(BaseModel):
    def __init__(self,
                 num_freqs,
                 look_ahead,
                 sequence_model,
...
class Two_Stage_Residual_FullSubNet_Small(BaseModel):
    def __init__(self,
                 num_freqs,
                 look_ahead,
                 sequence_model,
        ...
        self.complex_model = Complex_FullBandNet(
            num_freqs=num_freqs,
            look_ahead=look_ahead,
            sequence_model=sequence_model,
...
class Two_Stage_without_fc_FullSubNet_Small(BaseModel):
    def __init__(self,
                 num_freqs,
                 look_ahead,
                 sequence_model,
    ...
    def forward(self, noisy_complex):
        ...
        complex_input = torch.cat([enhanced_real, enhanced_imag], dim=1)  # [B, 2, F, T]
        cIRM = self.complex_model(complex_input)
...
class Two_Stage_AmpAttenion_without_fc_FullSubNet(BaseModel):
    def __init__(self,
                 num_freqs,
                 look_ahead,
                 sequence_model,
    ...
    def forward(self, noisy_complex):
        ...
        real_output = self.real_model(enhanced_real)
        imag_output = self.imag_model(enhanced_imag)
        cIRM = torch.cat([real_output, imag_output], dim=1)  # [B, 2, F, T]

        return IRM, cIRM


class Two_Stage_Residual_AmpAttenion_FullSubNet(BaseModel):
    def __init__(self,
                 num_freqs,
                 look_ahead,
                 sequence_model,
    ...
    def forward(self, noisy_complex):
        ...
        real_output = self.real_model(real_input) + real_input
        imag_output = self.imag_model(imag_input) + imag_input

        output = torch.cat([real_output, imag_output], dim=1)  # [B, 2, F, T]

        return IRM, output


class Two_Stage_Residual_AmpAttenion_without_fc_FullSubNet(BaseModel):
    def __init__(self,
                 num_freqs,
                 look_ahead,
                 sequence_model,
    ...
    def forward(self, noisy_complex):
        ...
        real_output = self.real_model(enhanced_real) + enhanced_real
        imag_output = self.imag_model(enhanced_imag) + enhanced_imag
        output = torch.cat([real_output, imag_output], dim=1)  # [B, 2, F, T]
