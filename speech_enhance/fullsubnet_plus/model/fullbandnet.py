...
class Complex_FullBandNet(BaseModel):
    def __init__(
            self,
            num_freqs,
            hidden_size,
        ...
        self.fullband_model = Complex_SequenceModel(
            input_size=num_freqs,
            output_size=num_freqs,
            hidden_size=hidden_size,
            num_layers=3,
    ...
    def forward(self, noisy_mag):
        ...
        fb_input = self.norm(noisy_mag).reshape(batch_size, num_channels * num_freqs, num_frames)  # [B, 2 * F, T]
        # fb_input = self.channel_attention(fb_input)
        output = self.fullband_model(fb_input).reshape(batch_size, 2, num_freqs, num_frames)
